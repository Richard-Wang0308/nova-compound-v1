...
# from utils.molecules import compute_maccs_entropy  # Not needed with num_molecules_boltz=1 (entropy requires multiple molecules)
...
class BoltzWrapper:
    def __init__(self):
        ...
        # Cache for protein sequences (target changes weekly, 140 epochs)
        self._protein_sequence_cache: Dict[str, str] = {}
        self._last_target = None


    def preprocess_data_for_boltz(self, valid_molecules_by_uid: dict, score_dict: dict, final_block_hash: str) -> None:
        # Get protein sequence with caching (target changes weekly, 140 epochs)
        current_target = self.subnet_config['weekly_target']
        if current_target != self._last_target or current_target not in self._protein_sequence_cache:
            # Cache miss or target changed - fetch and cache
            self._protein_sequence_cache[current_target] = get_sequence_from_protein_code(current_target)
            self._last_target = current_target
            bt.logging.debug(f"Cached protein sequence for target: {current_target}")
        
        self.protein_sequence = self._protein_sequence_cache[current_target]
        ...
        for uid, valid_molecules in valid_molecules_by_uid.items():
            ...
            num_available = len(valid_molecules.get('smiles', []))
            num_requested = self.subnet_config['num_molecules_boltz']
            
            if num_available == 0:
                bt.logging.warning(f"No valid molecules available for UID {uid}")
            ...
            if self.subnet_config['sample_selection'] == "random":
                ...
                k = min(num_requested, num_available)
                unique_indices = rng.sample(range(num_available), k=k)
                boltz_candidates_smiles = [valid_molecules['smiles'][i] for i in unique_indices]
            elif self.subnet_config['sample_selection'] == "first":
                boltz_candidates_smiles = valid_molecules['smiles'][:num_requested]
            else:
                bt.logging.error(f"Invalid sample selection method: {self.subnet_config['sample_selection']}")
                score_dict[uid]["entropy_boltz"] = None
                continue

            # Entropy calculation disabled - not meaningful with num_molecules_boltz=1
        ...
        for smiles, ids in self.unique_molecules.items():
            if not ids:
                bt.logging.warning(f"Empty ids list for smiles {smiles}, skipping")
                continue
...
properties:
    ...
    def postprocess_data(self, score_dict: dict) -> None:
        ...
        for smiles, id_list in self.unique_molecules.items():
            if not id_list:
                bt.logging.warning(f"Empty id_list for smiles {smiles}, skipping")
                continue
            ...
            if mol_idx not in scores:
                scores[mol_idx] = {}
            # Check if results directory exists before listing files
            if not os.path.exists(results_path):
                bt.logging.warning(f"Results path does not exist for mol_idx {mol_idx}: {results_path}")
                continue
        ...
        for smiles, id_list in self.unique_molecules.items():
            for uid, mol_idx in id_list:
                ...
                # Check if scores exist and metric key is present
                if mol_idx not in scores:
                    bt.logging.warning(f"No scores found for mol_idx {mol_idx}")
                    continue
                if self.subnet_config['boltz_metric'] not in scores[mol_idx]:
                    bt.logging.warning(f"Metric '{self.subnet_config['boltz_metric']}' not found in scores for mol_idx {mol_idx}")
                    continue
