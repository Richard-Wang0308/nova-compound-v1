...
# Cache for HuggingFace dataset (expensive to load, used frequently)
_protein_dataset_cache = None
...
def get_sequence_from_protein_code(protein_code: str) -> str:
    ...
    try:
        global _protein_dataset_cache
        if _protein_dataset_cache is None:
            _protein_dataset_cache = load_dataset("Metanova/Proteins", split="train")
            bt.logging.debug("Loaded and cached HuggingFace protein dataset")
        
        dataset = _protein_dataset_cache
...
def get_challenge_params_from_blockhash(block_hash: str, weekly_target: str, num_antitargets: int, include_reaction: bool = False) -> dict:
    ...
    # Load huggingface protein dataset (use cache if available)
    global _protein_dataset_cache
    try:
        if _protein_dataset_cache is None:
            _protein_dataset_cache = load_dataset("Metanova/Proteins", split="train")
            bt.logging.debug("Loaded and cached HuggingFace protein dataset for challenge params")
        dataset = _protein_dataset_cache
    except Exception as e:
